    ...
    <div class="layout-container">
      ...
      <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
        ...
        <ul class="menu-inner py-1">
          <!-- Dashboards -->
          <li class="menu-item">
            <a href="Admin_dashboard.php" target="_blank" class="menu-link">
          ...
          <li class="menu-item active open">
            ...
            <ul class="menu-sub">
              <li class="menu-item active">
                <a href="Judge_grading_system.php" target="_blank" class="menu-link">
      ...
      <div class="layout-page">
                      ...
                      <div class="d-flex">
                        ...
                        <div class="flex-grow-1">
                          ...
                          <small class="text-muted">
                            <?php
                            $sql_for_admin = "SELECT `account_type` FROM `organizer` WHERE `account_type` = 'Judge'";
        ...
        <div class="content-wrapper">
          <!-- Content -->
          <?php
          $query_fetch_contestants = "SELECT `contestant_id`, `contestant_fullname`, `contestant_no`, `contestant_address`, `archived` FROM `add_contestant`";
          $result = $CONNECTION->query($query_fetch_contestants);

          if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              // Assuming you fetch contestant_id from some user input or selection, e.g., a dropdown
              $contestant_id = $row['contestant_id']; // Replace this logic with actual selection mechanism
              break; // Remove this if you want to handle multiple contestants
            }
          } else {
            echo "<p>Error: No active contestants found.</p>";
            exit;
          }

          // Check if the contestant_id is set in the URL
          if (isset($_GET['contestant_id'])) {
            $contestant_id = intval($_GET['contestant_id']);
          } else {
            // Default to contestant ID 3 if not set
            $contestant_id = 3;
          }

          // Fetch contestant details based on the dynamic contestant ID
          $query_fetch_contestants = "SELECT `contestant_id`, `contestant_fullname`, `contestant_no`, `contestant_address`, `archived` FROM `add_contestant` WHERE `contestant_id` = $contestant_id";
          $result = $CONNECTION->query($query_fetch_contestants);

          if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            // You can use $row to get contestant details
          } else {
            echo "<p>Error: No contestant found with ID $contestant_id.</p>";
            exit;
          }
          ...
          // Check if contestant_id exists in add_contestant table
          $stmt_check_contestant = $CONNECTION->prepare("SELECT 1 FROM add_contestant WHERE contestant_id = ?");
          $stmt_check_contestant->bind_param("i", $contestant_id);
          $stmt_check_contestant->execute();
          $stmt_check_contestant->store_result();

          if ($stmt_check_contestant->num_rows == 0) {
            echo "<p>Error: Contestant with ID $contestant_id does not exist.</p>";
          } else {
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            ...
            <?php
              ...
              $sql = "SELECT `total_score` FROM `ranking_system` WHERE `archive` = 0 LIMIT 0, 5";  // adjust the query based on your table structure and requirements
              ...
              <span class="fw-medium d-block">
                Total Score Earned:
                <?php
                echo isset($totalScore) ? $totalScore . '%' : '0%';
                ?>
                % of 100%
              </span>
            </div>
            <ul class="nav nav-tabs">
              <li class="nav-item"><a class="nav-link" href="../judge/Judge_grading_system.php">Contestant 101</a></li>
              ...
              <li class="nav-item"><a class="nav-link active" href="../judge/Contestant5.php">Contestant 105</a></li>
