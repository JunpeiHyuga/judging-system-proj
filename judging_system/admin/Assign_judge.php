                ...
                <div class="card mb-4">
                  ...
                  <div class="card-body">
                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                      <div class="mb-3">
                        <label class="form-label" for="basic-icon-default-fullname">Judges: </label>
                        <div class="input-group input-group-merge">
                          <span id="basic-icon-default-fullname2" class="input-group-text"><i class="bx bx-user"></i></span>
                          <select name="judge_id" id="judge" class="form-control">
                            <!-- Populate options with PHP -->
                            <?php
                            // Assuming connection to MySQLi is stored in $conn
                            $result = $CONNECTION->query("SELECT judges_id, fullname FROM add_judges");
                            while ($row = $result->fetch_assoc()) {
                              echo "<option value='{$row['judges_id']}'>{$row['fullname']}</option>";
                            }
                            ?>
                          </select>
                        </div>
                      ...
                      <div class="mb-3">
                        ...
                        <div class="input-group input-group-merge">
                          <span id="basic-icon-default-fullname2" class="input-group-text"><i class="bx bx-calendar-event"></i></span>
                          <select name="event_id" id="judge" class="form-control">
                            <!-- Populate options with PHP -->
                            <?php
                            // Assuming connection to MySQLi is stored in $conn
                      ...
                      <div class="mb-3">
                        <label class="form-label" for="basic-icon-default-fullname">Criteria: </label>
                        <div class="input-group input-group-merge">
                          <span id="basic-icon-default-fullname2" class="input-group-text"><i class="bx bx-phone"></i></span>
                          <select name="criteria_id" id="judge" class="form-control">
                            <!-- Populate options with PHP -->
                            <?php
                            // Assuming connection to MySQLi is stored in $conn
                            $result = $CONNECTION->query("SELECT `criteria_id`, `criteria_1_name`, `criteria_1_value`, `criteria_2_name`, `criteria_2_value`, `criteria_3_name`, `criteria_3_value`, `criteria_4_name`, `criteria_4_value` FROM `add_criteria` ");
                            while ($row = $result->fetch_assoc()) {
                              echo "<option value='{$row['criteria_id']}'>{$row['criteria_id']}</option>";
                            }
                            ?>
                          </select>
                        </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  // Prepare and bind
  $stmt = $CONNECTION->prepare("INSERT INTO assign_judges (judges_id, event_id, criteria_id) VALUES (?, ?, ?)");
  $stmt->bind_param("iii", $judge_id, $event_id, $criteria_id);
