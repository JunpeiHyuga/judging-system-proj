              ...
              <div class="col-xl">
                <div class="card mb-4">
                  ...
                  <div class="card-body">
                    ...
                    if ($id) {
                      $query = "SELECT * FROM add_events WHERE event_id = ?";
                    ...
                    <input type="hidden" name="id" value="<?php echo isset($id) ? $id : ''; ?>">
                      <div class="mb-3">
                        ...
                        <div class="input-group input-group-merge">
                          ...
                          <input type="text" class="form-control" id="basic-icon-default-fullname" placeholder="Event Name" aria-label="Event Name" aria-describedby="basic-icon-default-fullname2" name="event_name" value="<?php echo isset($event_name) ? $event_name : ''; ?>" />
                        </div>
                      </div>
                      <div class="mb-3">
                        ...
                        <div class="input-group input-group-merge">
                          ...
                          <input type="text" id="basic-icon-default-company" class="form-control" placeholder="Status" aria-label="yyyy - yyyy" aria-describedby="basic-icon-default-company2" name="status" value="<?php echo isset($status) ? $status : ''; ?>" />
                        </div>
                      </div>
                  </div>
...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
  ...
  if (isset($_POST['id'])) {
    $id = $_POST['id'];
    $sqlquery = "UPDATE add_events SET event_name = ?, status = ? WHERE event_id = ?";
    $stmt = mysqli_prepare($CONNECTION, $sqlquery);
    mysqli_stmt_bind_param($stmt, 'ssi', $event_name, $status, $id);
  } else {
    // If no ID, perform an insert
    $sqlquery = "INSERT INTO add_events (event_name, status) VALUES (?, ?)";
    $stmt = mysqli_prepare($CONNECTION, $sqlquery);
    mysqli_stmt_bind_param($stmt, 'ss', $event_name, $status);
  }
  ...
  if (mysqli_stmt_execute($stmt)) {
    echo "<script>
      $(document).ready(function() {
        ...
        $('#successModal').modal('show');
        window.location = '../admin/Event_archive.php'; // Redirect to dashboard or appropriate page
      });
    </script>";
  } else {
